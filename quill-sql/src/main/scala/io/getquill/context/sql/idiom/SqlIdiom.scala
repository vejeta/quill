...
trait SqlIdiom extends Idiom {
  ...
  implicit def propertyTokenizer(implicit valueTokenizer: Tokenizer[Value], identTokenizer: Tokenizer[Ident], strategy: NamingStrategy): Tokenizer[Property] = {
    ...
    Tokenizer[Property] {
      case Property(ast, "isEmpty") => stmt"${unnest(ast).token} IS NULL"
      case Property(ast, "nonEmpty") => stmt"${unnest(ast).token} IS NOT NULL"
      case Property(ast, "isDefined") => stmt"${unnest(ast).token} IS NOT NULL"
